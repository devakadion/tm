...
export default function App() {
  ...
  return (
    <main className="flex h-screen w-full flex-col overflow-hidden bg-stone-950 px-4 font-sans text-sm text-stone-200">
      <style>{`
          ...
          {!entered ? (
            ...
            >
              <div className="flex w-full max-w-fit flex-col items-center gap-6">
                <Logo type={settings.logoType} animated={settings.logoAnimated} />
            ...
            >
              ...
              <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
                {activeTab.label === 'features' && (
                  <FeatureTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'bugs' && (
                  <BugTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'refactor' && (
                  <RefactorTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'snippets' && (
                  <SnippetTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'tasks' && (
                  <TaskTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'notes' && (
                  <NoteTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'all' && (
                  <AllTab lines={allLines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
                {activeTab.label === 'settings' && (
                  <SettingsTab
                    logoType={settings.logoType}
                    setLogoType={settings.setLogoType}
                    logoAnimated={settings.logoAnimated}
                    setLogoAnimated={settings.setLogoAnimated}
                  />
                )}
                {!knownLabels.includes(activeTab.label) && (
                  <AllTab lines={activeTab.lines} onCommand={runCommand} logoType={settings.logoType} logoAnimated={settings.logoAnimated} />
                )}
              </div>
            </motion.div>
          )}
